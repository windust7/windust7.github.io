...
import torch
import json
from pipelines.pipeline_sc import SelfCrossPipeline
import numpy as np
import random
...
def main():
    if model_choice == "SD15":
        pipe = SelfCrossPipeline.from_pretrained(SD15_VERSION).to("cuda:0")
        attn_res = (16, 16)
        max_iter_to_alter = 25
        run_initno = True
    elif model_choice == "SD21":
        pipe = SelfCrossPipeline.from_pretrained(SD21_VERSION).to("cuda:0")
        attn_res = (24, 24)
        max_iter_to_alter = 40
        run_initno = True  # for sd2 initno and attend&excite don't work well
    else:
    ...
    for PROMPT in prompts:
        ...
        path = './outputs/selfcross_noise_{}/{}/{}'.format(num_initial_noises, dataset, PROMPT)
        ...
        for SEED in seeds:
            ...
            images = pipe(prompt=PROMPT,token_indices=token_indices,guidance_scale=7.5,generator=generator,
                          ...
                          result_root=result_root,K=16,seed=SEED,run_sd=run_sd,run_initno=run_initno, num_initial_noises=num_initial_noises).images
