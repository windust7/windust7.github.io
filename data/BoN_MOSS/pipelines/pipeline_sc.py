...
class SelfCrossPipeline(DiffusionPipeline, TextualInversionLoaderMixin, StableDiffusionLoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
    ...
    def fn_initno(
        ...
        return optimized_latents, optimization_succeed, cross_attn_loss.detach() + self_cross_attn_loss.detach(), log_var.detach(), mu.detach()
                    ...
                    if not run_bon_sc:
                        ...
                        for n_idx, lat_ in enumerate(latents_list):
                            ...
                            for latent, index, text_embedding in zip(lat_, indices, text_embeddings):
                                ...
                                joint_loss, cross_attn_loss, self_cross_attn_loss = self.fn_augmented_compute_losss(indices=index, K=K, attention_res=attn_res[0], return_raw=True)
                                self.cross_attention_maps_cache = None
                                
                                local_loss_sum += joint_loss.item()
