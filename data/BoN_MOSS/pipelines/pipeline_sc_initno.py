...
class SelfCrossInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, StableDiffusionLoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    ...
    def fn_initno(
        ...
        if (log_var is not None) and (mu is not None):
            log_var, mu = log_var.clone().detach(), mu.clone().detach()
            latents = (latents - mu) / (torch.exp(0.5 * log_var))
        else:
            log_var, mu = torch.zeros_like(latents), torch.zeros_like(latents)
        ...
        if cross_attn_loss.item() > 0.5:
            self_cross_attn_loss_item = self_cross_attn_loss.item() + 1e6
        else:
            self_cross_attn_loss_item = self_cross_attn_loss.item()
        return optimized_latents, optimization_succeed, cross_attn_loss.detach() + self_cross_attn_loss_item, log_var.detach(), mu.detach()
    ...
    def __call__(
        ...
        run_bon: bool = True,
        total_function_call: int = 50,
        bon_optim: int = 10,
        
        run_bon_sc: bool = False,
        num_initial_noises: int = 10,
        switch_step: int = 0,
    ):
        r"""
            ...
            if not run_bon:
                ...
                with torch.enable_grad():
                    ...
                    for round in range(max_round):
                        optimized_latents, optimization_succeed, cross_self_attn_loss, _, _ = self.fn_initno(
                            latents=latents,
                ...
                if bon_optim == 0:
                    without_optimization = True
                    max_round = total_function_call * 2
                else:
                    without_optimization = False
                    max_round = int(total_function_call / bon_optim)
                
                with torch.enable_grad():
                    optimized_latents_pool = []
                    for round in range(max_round):
                        latents = torch.randn_like(latents)
                        optimized_latents, optimization_succeed, cross_self_attn_loss, log_var, mu = self.fn_initno(
                            latents=latents,
                            indices=token_indices[0],
                            text_embeddings=prompt_embeds,
                            max_step=bon_optim,
                        ...
                        optimized_latents_pool.append(
                            (cross_self_attn_loss, round, optimized_latents.clone(), latents.clone(), optimization_succeed, log_var, mu))
                    for score, _round, _optimized_latent, _latent, _optimization_succeed, log_var, mu in optimized_latents_pool:
                        print(f'Optimization_succeed: {_optimization_succeed} - Attn score: {score.item():0.4f} - Round: {_round}')
                    optimized_latents_pool.sort()
                    latents = optimized_latents_pool[0][2]
                    log_var, mu = optimized_latents_pool[0][5], optimized_latents_pool[0][6]
                    latents, optimization_succeed, cross_self_attn_loss, _, _ = self.fn_initno(
                            latents=latents,
                            indices=token_indices[0],
                            text_embeddings=prompt_embeds,
                            max_step=50,
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    if not run_bon_sc:
                        ...
                        if single_latent_mode:
                            best_latents = best_latents.clone().detach().requires_grad_(True)
                            updated_best = []
                            for latent, index, text_embedding in zip(best_latents, indices, text_embeddings):
                                latent = latent.unsqueeze(0)
                                text_embedding = text_embedding.unsqueeze(0)
                                ...
                                updated_best.append(latent)
                            best_latents = torch.cat(updated_best, dim=0)
                        
                        else:
                            new_latents_list = []
                            losses_for_this_step = []
                            ...
                            for n_idx, lat_ in enumerate(latents_list):
                                ...
                                for latent, index, text_embedding in zip(lat_, indices, text_embeddings):
                                    latent = latent.unsqueeze(0)
                                    text_embedding = text_embedding.unsqueeze(0)
                                    ...
                                    # if (cross_attn_loss > 0.2 or self_cross_attn_loss > 0.3) and not run_sd:
                                    if i < max_iter_to_alter and (i == 10 or i == 20 ) and (cross_attn_loss > 0.2 or self_cross_attn_loss > 0.3) and not run_sd:
                                        joint_loss, cross_attn_loss, self_cross_attn_loss, latent = self._perform_iterative_refinement_step(
                                            latents=latent,
                                            indices=index,
                                    ...
                                    local_loss_sum += cross_attn_loss.item()
                                    ...
                                    updated_sub_latents.append(latent)
                                
                                lat_ = torch.cat(updated_sub_latents, dim=0)
                                new_latents_list.append(lat_)
                                losses_for_this_step.append(local_loss_sum)
                                
                            latents_list = new_latents_list
                            
                            if i == switch_step:
                                best_idx = int(np.argmin(losses_for_this_step))
                                best_latents = latents_list[best_idx]
                                single_latent_mode = True
                                logging.info(
                                    f"[run_bon_sc] switch_step reached at i={i}, pick best idx={best_idx}, loss={losses_for_this_step[best_idx]:.4f}"
                                )
                ...
                if run_bon_sc:
                    if single_latent_mode:
                        latent_model_input = torch.cat([best_latents] * 2) if do_classifier_free_guidance else best_latents
                        ...
                        best_latents = self.scheduler.step(noise_pred, t, best_latents).prev_sample
                        ...
                        for n_idx, lat_ in enumerate(latents_list):
                            print(f"\n[run_bon_sc] Processing noise {n_idx} at timestep {i}")
                            latent_model_input = torch.cat([lat_] * 2) if do_classifier_free_guidance else lat_
                            latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                            ...
                            latents_list[n_idx] = self.scheduler.step(noise_pred, t, lat_).prev_sample
                            # 주의!!!!!!!! 다른 scheduler로 바뀌면 거기에 맞춰서 확인해야함!!!!!!!!!!!!!!!
                            if n_idx < len(latents_list) - 1:
